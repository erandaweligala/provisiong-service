...
public class WebClientConfig {
    ...
    /**
     * WebClient for Accounting Service API calls.
     *
     * FIX #1: Added baseUrl() so relative URIs like /patchStatus/{user}/{status}
     *         are resolved correctly. Previously the client had no base URL and
     *         every call would fail with a connection error.
     *
     * FIX #3: Removed responseTimeout from HttpClient — timeout is now handled
     *         solely at the HttpClient level via ReadTimeoutHandler, which throws
     *         IOException and is correctly caught by CoAManagementService#shouldRetry.
     *         Using both responseTimeout AND the Reactor .timeout() operator causes
     *         duplicate, conflicting timeout behaviour.
     */
    @Bean
    public WebClient accountingApiWebClient() {

        HttpClient httpClient = HttpClient.create()
                .option(ChannelOption.CONNECT_TIMEOUT_MILLIS, accountingConnectTimeout)
                .doOnConnected(conn ->
                        conn.addHandlerLast(new ReadTimeoutHandler(accountingReadTimeout, TimeUnit.MILLISECONDS))
                                .addHandlerLast(new WriteTimeoutHandler(accountingReadTimeout, TimeUnit.MILLISECONDS)));

        log.info("Configured Accounting API WebClient with baseUrl={}, connect-timeout={}ms, read-timeout={}ms",
                accountingServiceUrl, accountingConnectTimeout, accountingReadTimeout);

        return WebClient.builder()
                .baseUrl(accountingServiceUrl)                              // FIX #1: was missing
                .clientConnector(new ReactorClientHttpConnector(httpClient))
                .build();
    }
}