 ...
 * the {@code api} tag that names the catalog entry serving each request, so the
 * dashboard can group by endpoint without depending on URL paths.</p>
 *
 * <ul>
 *   <li>{@code http_server_requests_seconds_count} carrying an extra {@code api}
 *       tag - the source of the availability figure;</li>
 *   <li>{@code api_endpoint_info}, a constant 1 per catalogued endpoint, so
 *       every endpoint appears on the dashboard from startup rather than only
 *       after its first request.</li>
 * </ul>
 */
@Slf4j
@Configuration
...
public class ApiMonitoringConfig {
     ...
     * last restart is simply missing from Prometheus. This series makes the
     * catalog itself visible, which is what lets the dashboard list every
     * endpoint and show "no data" instead of silently dropping the row.</p>
     */
    @Bean
